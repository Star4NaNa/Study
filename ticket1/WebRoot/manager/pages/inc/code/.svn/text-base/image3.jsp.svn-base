...
int[] style = {Font.BOLD,Font.ITALIC,Font.PLAIN,Font.ROMAN_BASELINE,Font.CENTER_BASELINE}; 
g.setFont(new Font(fontTypes[random.nextInt(fontTypes.length)],style[random.nextInt(style.length)],17));
//画边框 
...
for (int i=0;i<4;i++){
    ...
    String rand=base.substring(number,number+1);
    sRand+=rand;
    // 将认证码显示到图象中
    g.setColor(Color.white);
    g.drawString(rand,18*i+6,16);
}
